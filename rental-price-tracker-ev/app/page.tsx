...
export default function Home(){
  ...
  async function loadCalendar(month:string,oagCode:string){
    ...
    // Conservative concurrency so one bad/slow day does not break the whole calendar.
    const batchSize=2;
    try{
      for(let i=0;i<dates.length;i+=batchSize){
        ...
        const responses=await Promise.all(batch.map(async date=>{
          try{
            const q=new URLSearchParams({date,oagCode,pickupTime:"08:00",minAge:"30",countryCode:"CA"});
            const r=await fetch(`/api/hertz/calendar-day?${q}`,{cache:"no-store"});
            const d=await r.json();
            return r.ok?d:{date,available:false,total:null,daily:null,currency:"CAD",vehicleName:null,evCount:0,error:d?.error||`Erreur ${r.status}`};
